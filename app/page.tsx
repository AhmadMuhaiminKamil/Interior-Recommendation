...
export default function Home() {
  ...
  // FIX: Panggil server action logout() — satu-satunya cara hapus cookie httpOnly
  // document.cookie tidak bisa menyentuh cookie httpOnly dari sisi browser
  ...
  return (
    <main className="min-h-screen p-4 md:p-8 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 font-sans">
      ...
      {/* Keyframe spin untuk loading spinner tombol logout */}
      <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>

      <div className="max-w-4xl mx-auto">

        {/* Tombol Logout — full inline style */}
          ...
          <button
            ...
            onMouseEnter={e => {
              if (!loggingOut) {
                ...
                btn.style.background = 'linear-gradient(135deg, rgba(168, 85, 247, 0.5), rgba(236, 72, 153, 0.5))';
                btn.style.color = '#ffffff';
              }
            }}
            onMouseLeave={e => {
              const btn = e.currentTarget as HTMLButtonElement;
              btn.style.background = 'linear-gradient(135deg, rgba(88, 28, 135, 0.2), rgba(131, 24, 67, 0.2))';
        ...
        {/* Kontainer Utama */}
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-8 rounded-3xl shadow-2xl shadow-purple-900/20 mb-10 border border-slate-700/50 backdrop-blur-sm">
